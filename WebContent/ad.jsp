...
<html>
	...
	{ var n = 0; 
	   ...
	   document.messageForm.action="clearad.do"; 
	   document.messageForm.submit(); 
	   }
	   }
	} function action2() 
	{ 
		   document.messageForm.action="admoveto.do"; 
		   document.messageForm.submit();
	}
	function changepage(s){
		
	   document.messageForm.action="goad2.do?pageNo="+s; 
	   document.messageForm.submit();
	}
</script>
		...
		<form name="messageForm" id="oFormMessage" method="post">
			<div class="ContentWp">
				<div class="ContentThemeWp">
					<div class="gTitle">
						<b class="mTT"> </b> &nbsp;&nbsp;(共&nbsp;
						<b id="oTotal">${fn:length(adEmailBeanList)}</b>&nbsp;封)
					</div>
					<div class="gToolbar gTbrTop">
						...
						<div class="Extra">
							<span class="Txt"><span class="Unable"><a
									href="goad2.do?pageNo=1">首页</a>
							</span>&nbsp;&nbsp; <span class="Unable"><a
									href="goad2.do?pageNo=${adPageNo - 1 }">上页</a>
							</span> &nbsp;&nbsp; <span class="Unable"> <a
									href="goad2.do?pageNo=${adPageNo + 1 }">下页</a>
							</span>&nbsp;&nbsp; <span class="Unable"><a
									href="goad2.do?pageNo=${adTotalPages }">末页</a>
							</span> </span>
							<select class="SelA" style="margin-right: 0px" name="pageNo"
								onchange="changepage(this.value)">
								<c:forEach var="t" begin="1" end="${adTotalPages}">
									<option value="${t }">
										${t }/${adPages }
									</option>
								</c:forEach>
								...
								<tbody>
									...
									<logic:present name="adEmailBeanList" scope="session">
										...
										<logic:notEmpty name="adEmailBeanList">
											<c:forEach items="${adEmailBeanList}" var="t">
												<tr class="I_Mark0">
													<td class="Ibx_Td_F">
														&nbsp;
													</td>
													...
													<td class="Ibx_Td_From">
														<a href="#"
															onclick="javascript:changeframe('', '', 'admailtemp.jsp?emailid=${t.email.emailid }'); return false;">${t.email.recipients
															}</a>
													</td>
													...
													<td align="left">
														<a
															onclick="javascript:changeframe('', '', 'admailtemp.jsp?emailid=${t.email.emailid }'); return false;"
