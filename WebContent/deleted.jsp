...
<html>
	...
	{ 
		...
		if( n == 0 ) {
			...
			if(confirm("是否将此封邮件删除?（删除不能恢复！）")){
			   document.messageForm.action="deletedmaildeleting.do"; 
	...
	{ 
		...
		if( n == 0 ) {
		   ...
		   document.messageForm.action="deletedchangeFlag.do"; 
		   document.messageForm.submit();
		}
	}
	function action3() 
	{ 
		...
		if( n == 0 ) {
		   ...
		   document.messageForm.action="deletedmoveto.do"; 
		   document.messageForm.submit();
		 }
	}
	function action4() 
	{ 
	   document.messageForm.action="deletedqueryfrom.do"; 
	   document.messageForm.submit();
	}
	function changepage(s){
		
	   document.messageForm.action="godeleted2.do?pageNo="+s; 
	   document.messageForm.submit();
	}
</script>
		...
		<form name="messageForm" id="oFormMessage" method="post">
			<div class="ContentWp">
				<div class="ContentThemeWp">
					<div class="gTitle">
						<b class="mTT"> </b> &nbsp;&nbsp;(共&nbsp;
						<b id="oTotal">${fn:length(deletedEmailBeanList)}</b>&nbsp;封)
					</div>
					<div class="gToolbar gTbrTop">
						...
						<div class="Extra">
							<span class="Txt"><span class="Unable"><a
									href="godeleted2.do?pageNo=1">首页</a>
							</span>&nbsp;&nbsp; <span class="Unable"><a
									href="godeleted2.do?pageNo=${deletedPageNo - 1 }">上页</a>
							</span> &nbsp;&nbsp; <span class="Unable"> <a
									href="godeleted2.do?pageNo=${deletedPageNo + 1 }">下页</a>
							</span>&nbsp;&nbsp; <span class="Unable"><a
									href="godeleted2.do?pageNo=${deletedTotalPages }">末页</a>
							</span> </span>
							<select class="SelA" style="margin-right: 0px" name="pageNo"
								onchange="changepage(this.value)">
								<c:forEach var="t" begin="1" end="${deletedTotalPages}">
									<option value="${t }">
										${t }/${deletedTotalPages }
									</option>
								</c:forEach>
...
选中该组所有邮件"
							...
							<table class="Ibx_gTable Ibx_gTable_Con" id="oTableCOUNT0">
								<tbody>
									...
									<logic:notPresent name="deletedEmailBeanList" scope="session">
										<tr class="I_Mark0">
											<td align="center">
												对不起，当前没有邮件！
											</td>
									...
									<logic:present name="deletedEmailBeanList" scope="session">

										<logic:empty name="deletedEmailBeanList">
											<tr class="I_Mark0">
												<td align="center">
													对不起，查询错误，List为空！
												</td>
										...
										<logic:notEmpty name="deletedEmailBeanList">
											<c:forEach items="${deletedEmailBeanList}" var="t">
												<tr class="I_Mark0">
													<td class="Ibx_Td_F">
														&nbsp;
													</td>
													...
													<td class="Ibx_Td_From">
														<a href="#"
															onclick="javascript:changeframe('', '', 'deletedmailtemp.jsp?emailid=${t.email.emailid }'); return false;">${t.email.recipients
															}</a>
													</td>
													...
													<td class="Ibx_Td_Subject" align="left">
														<a
															onclick="javascript:changeframe('', '', 'deletedmailtemp.jsp?emailid=${t.email.emailid }'); return false;"
...
javascript:void(0)" type="read">已读</a>&nbsp;-&nbsp;
					...
					<div class="gToolbar gTbrBtm">
						<input type="button" class="Btn BtnNml ImpBtn" value="删 除" />
						<select class="Sel">
							...
							<option value="label0:5" class="fnt_M ark5">
								&nbsp;&nbsp;亲友邮件
							</option>
						...
						<select class="Sel">
							...
							<option value="replied:true">
								&nbsp;&nbsp;已回复
							</option>
							<option value="forwarded:true">
								&nbsp;&nbsp;已转发
							</option>
							<option value="none" class="SelOptLine">
								-------------
